    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                if (choice == "1")
                {
                    if (AdjectiveRepository.TryGetRandom(out var word))
                    {
                ...
                if (choice == "2")
                {
                    ...
                    if (AdjectiveRepository.TryGetRandomByLetter(letter, out var wordByLetter))
                    {
                ...
                if (choice == "3")
                {
                    ...
                    var exists = AdjectiveRepository.All
                    ...
                    var repoEntry = AdjectiveRepository.All
        ...
        private static void PrintColoredWord(string word)
        {
            ...
            // Adjective only (no article logic)
        ...
        private static void PrintExampleSentences(string word, int count = Constants.NumberOfRandomExampleSentences)
        {
            ...
            var sentences = AdjectiveRepository.GetSentencesForWord(word, count);
        ...
        private static void PrintDescription(string word)
        {
            ...
            var description = AdjectiveRepository.GetDescriptionForWord(word);
        ...
        private static string ResolveToCanonical(string input)
        {
            ...
            // For adjectives we expect the adjective itself (no article).
